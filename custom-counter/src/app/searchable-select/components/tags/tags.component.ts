...
export class TagsComponent implements OnInit, ControlValueAccessor {
    ...
    add(event: MatChipInputEvent): void {
        ...
        if (!this.matAutocomplete.isOpen) {
            ...
            if (this.viewData.tags$.getValue().find(x => x.text.toLowerCase() === value.toLowerCase()))
                return;

            // Add our fruit
            if ((value || '').trim()) {
                this.viewData.tags$.getValue().push({ id: "", text: value.trim() });
            }
    ...
    selected(event: MatAutocompleteSelectedEvent): void {
        ...
        this.viewData.tags$.getValue().push({ id: event.option.value.id, text: event.option.value.text });
    ...
    private _filter(value: any): string[] {
        ...
        return this.data.filter(x => x.text.toLowerCase().indexOf(filterValue) === 0);
    }
